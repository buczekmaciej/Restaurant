...
@section('body')
    ...
    <div class="h-screen">
        ...
        <div class="w-full h-full bg-[#141414CC] flex flex-col justify-center gap-5 px-40">
            ...
            <p class="text-white font-light w-[55%] leading-7 tracking-wide">Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolore velit nihil dicta fuga, inventore eveniet nam quae maxime obcaecati earum perferendis consectetur sunt omnis esse culpa libero deserunt quod saepe? Odio, fugit voluptatibus, doloremque sapiente ex ipsam nihil odit, inventore vitae in harum perferendis neque voluptate cupiditate ipsum dolore temporibus. Soluta quaerat minima iure! Ex cupiditate veniam quae
                voluptatibus
                accusantium.</p>
            <form action="{{ route('order.create') }}" class="flex flex-wrap gap-2 mt-6" method="POST">
                @csrf
                <p class="text-white w-full">Order online:</p>
                <input class="home-input" name="address[street]" placeholder="Street address" type="text" />
                <input class="home-input" list="cities-list" name="address[city]" placeholder="City" type="text" />
                <datalist id="cities-list">
                    @foreach ($locations as $address)
                        <option value="{{ $address->city }}"></option>
                    @endforeach
                </datalist>
                <button class="bg-orange-700 text-white rounded-lg px-6 cursor-pointer hover:bg-orange-800 outline-transparent">Select food</button>
            </form>
        </div>
    </div>
    <div class="px-40 py-20 flex flex-col gap-10">
        ...
        <div class="grid grid-cols-4 gap-9">
            @foreach ($locations as $address)
                <a class="hover:bg-zinc-200 p-3 rounded-lg" href="https://www.google.com/maps/search/{{ $address->street }},%20{{ $address->city }}" target="_blank">
                    <div class="flex gap-4 items-center">
                        ...
                        <div>
                            <p class="text-xl font-medium">{{ $address->city }}</p>
                            <p class="font-light text-lg">{{ $address->street }}</p>
                        </div>
                    </div>
                </a>
            @endforeach
